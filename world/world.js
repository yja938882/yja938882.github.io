...
$(function(){
  ...
  var gj = $.getJSON(myGeoJSONPath,function(data){
    ...
    for ( var i=0; i < pre_markers.length; ++i ) {
        //var marker = L.animatedMarker( [[pre_markers[i].lat, pre_markers[i].lng], [pre_markers[i].lat -20, pre_markers[i].lng-20]]);
        //marker.addTo(map);
        var myMovingMarker = L.Marker.movingMarker(
          [[pre_markers[i].lat, pre_markers[i].lng], [pre_markers[i].lat+3, pre_markers[i].lng],[pre_markers[i].lat, pre_markers[i].lng]],
            [400,400],{"loop":true, "autostart":false}).bindPopup( '<a href="' + pre_markers[i].name + '" target="_blank">' + pre_markers[i].name + '를 클릭하셨습니다'+ '</a>' ).addTo(map).on('click',function(){
              if(this.isRunning())
                this.stop();
              else
                this.start();
        ...
        ///L.marker( [pre_markers[i].lat, pre_markers[i].lng] ).bindPopup( '<a href="' + pre_markers[i].name + '" target="_blank">' + pre_markers[i].name + '를 클릭하셨습니다'+ '</a>' )
          //.addTo( map );
      }
